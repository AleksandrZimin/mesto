export default class FormValidator { 
   ...
   _switchButtonState() { // функция для кнопки
      if(this._isFormValid()) {  // если форма валидна, то кнопку разблокируем, если нет, то блокируем
         this._enableButton()
      } else {
         this.disableButton()
      }
   ...
   _isFormValid() { // проверка валидности формы
      return !this._formInputs.some(input => {
         return !input.validity.valid
      })
   }

   _enableButton() { // разблокирование кнопки
      this._formButton.classList.remove(this._validationConfig.inactiveButtonClass) // удаляем класс
      this._formButton.removeAttribute('disabled') // удаляем атрибут
   }

   disableButton() { // разблокирование кнопки
   ...
   _checkInputValid = (input) => {   // функция проверки валидности информации при вводе
   ...
   _hideError(errorSpan, input) { // функция скрытия ошибки
